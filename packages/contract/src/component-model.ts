...
const MODELS: Record<DevComponentName, ComponentPropDef[]> = {
  AnalyticsCharts: [
...
export function propsForComponent(component: DevComponentName): ComponentPropDef[] {
  return MODELS[component] ?? [];
}

export function formatPropsInterface(component: DevComponentName): string {
...
export function formatJsxUsage(storyId: string): string | null {
  ...
  if (lines.length === 0) {
    const optionalHints = propDefs
      ...
      .slice(0, 4)
      .map((p) => `  ${p.name}=…`);
    if (optionalHints.length === 0) return `<${component} />`;
    return `<${component}\n${optionalHints.join("\n")}\n/>`;
  }
  return `<${component}\n${lines.join("\n")}\n/>`;
}

export function reactModelForStory(storyId: string): {
