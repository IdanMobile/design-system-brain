...
(() => {
  ...
  function snapBoxSize(layer, axis) {
    ...
    if (((_b = layer.source) == null ? void 0 : _b.kind) === "figma" && ((_c = layer.source.dataset) == null ? void 0 : _c.figmaNodeType) === "ELLIPSE" && layer.box.width <= 48 && layer.box.height <= 48) {
      return Math.max(1, Math.round(v));
    }
  ...
  function gradientPaint(layer, width, height, opts) {
    if (!(opts == null ? void 0 : opts.preferCssAngle)) {
  ...
  function isFigmaReferenceRasterLayer(layer) {
    var _a, _b, _c;
    return Boolean(
      ((_a = layer.image) == null ? void 0 : _a.dataUrl) && ((_c = (_b = layer.source) == null ? void 0 : _b.dataset) == null ? void 0 : _c.figmaReferenceRaster)
    );
  }
  ...
  function figmaNativeNeedsWidthConstrainedWrap(layer, text) {
    var _a, _b, _c, _d;
    if (!isFigmaNativeTextLayer(layer)) return false;
    const resize = (_b = (_a = layer.source) == null ? void 0 : _a.dataset) == null ? void 0 : _b.figmaTextAutoResize;
    if (resize !== "WIDTH_AND_HEIGHT") return false;
    ...
    const lh = (_c = text.lineHeight) != null ? _c : text.font.size;
    if (layer.box.height > lh * 1.3) return false;
    const fontSize = Math.max(1, (_d = text.font.size) != null ? _d : 14);
    ...
    return trimmed.length > estCharsPerLine || trimmed.length > 40 && layer.box.height <= lh * 1.3;
  }
  ...
  function pinFigmaFlexCrossEndBareText(text, layer, parent) {
    ...
    if (align !== "left" && align !== "start" && align !== "right" && align !== "end") {
      return;
    }
    ...
    text.textAlignHorizontal = layer.text.direction === "rtl" || align === "right" || align === "end" ? "RIGHT" : "LEFT";
    void text.width;
  ...
  function figmaReferenceRasterPosition(layer, parentDocOrigin) {
    var _a;
    const local = {
    ...
    const absX = (ds == null ? void 0 : ds.figmaReferenceAbsX) != null ? Number(ds.figmaReferenceAbsX) : NaN;
  ...
  function isFigmaNativeEllipseSiblingLetter(layer, parent) {
    var _a, _b;
    ...
    if (((_a = layer.text.value) != null ? _a : "").length > 3) return false;
    return ((_b = parent == null ? void 0 : parent.children) != null ? _b : []).some((c) => c !== layer && isFigmaNativeEllipse(c));
  }
  function applyFigmaNativeCenteredGlyphPin(text, layer, parent) {
    var _a;
    const boxW = Math.max(1, snap(layer.box.width));
    const boxH = Math.max(1, snap(layer.box.height));
    if (((_a = layer.text) == null ? void 0 : _a.lineHeight) != null && layer.text.lineHeight > boxH + 0.25) {
  ...
  async function resolveFont(text, layer, parent) {
    ...
    const pickNearestInFamily = (family) => {
    ...
    if (((_b = layer == null ? void 0 : layer.source) == null ? void 0 : _b.kind) === "figma") {
      ...
      if ((ds == null ? void 0 : ds.figmaFontFamily) && (ds == null ? void 0 : ds.figmaFontStyle)) {
        const hasHebrew = /[\u0590-\u05FF]/.test((_c = text.value) != null ? _c : "");
        const families = hasHebrew && ds.figmaFontFamily === "Open Sans" ? ["Open Sans Hebrew"] : [ds.figmaFontFamily];
    ...
    const tryFamily = (family) => pickNearestInFamily(family);
  ...
  function applyTransform(node, layer) {
    ...
    if (((_c = layer.source) == null ? void 0 : _c.kind) === "figma" && (layer.vector || isFigmaFlipIconFrame(layer)) && (rt == null ? void 0 : rt.length) === 2 && ((_d = rt[0]) == null ? void 0 : _d.length) === 3 && ((_e = rt[1]) == null ? void 0 : _e.length) === 3) {
    ...
    if (!isMockFigmaRuntime() && isFigmaNativeEllipse(layer) && layer.box.width <= 48 && layer.box.height <= 48) {
      x = Math.round(layer.box.x);
      y = Math.round(layer.box.y);
    }
  ...
  function buildFills(paint, width, height, layer) {
    var _a;
  ...
  async function createTextNode(layer, parent) {
    ...
    if (((_A = layer.source) == null ? void 0 : _A.kind) === "figma" && figmaResize) {
      ...
      } else if (mode === "WIDTH_AND_HEIGHT" && figmaNativeNeedsWidthConstrainedWrap(layer, text)) {
        mode = "NONE";
        wrapBoxOverride = true;
      }
      if (mode) {
  ...
  function reaffirmChildBoxPositions(built, parent, parentDocOrigin = { x: 0, y: 0 }) {
    var _a, _b;
    if (isMockFigmaRuntime()) return;
    for (const { node, layer } of built) {
      ...
      if (node.type === "TEXT" && isFigmaNativeEllipseSiblingLetter(layer, parent)) {
        applyFigmaNativeCenteredGlyphPin(node, layer, parent);
      }
    }
  }
  function reaffirmTreeBoxPositions(node, layer, parent, docOrigin = { x: 0, y: 0 }) {
    var _a, _b, _c;
    if (isMockFigmaRuntime()) return;
    if (isFigmaReferenceRasterLayer(layer)) {
      ...
      node.x = pos.x;
      node.y = pos.y;
    } else if (!(((_a = layer.source) == null ? void 0 : _a.kind) === "figma" && ((_b = layer.source.dataset) == null ? void 0 : _b.figmaRelativeTransform))) {
      if (!(isLiveLabButtonBareLabel(layer, parent) && node.type === "TEXT")) {
        node.x = snap(layer.box.x);
        node.y = snap(layer.box.y);
      }
    ...
    if (!((_c = layer.children) == null ? void 0 : _c.length) || !("children" in node)) return;
  ...
  function isFigmaFlipIconFrame(layer) {
    var _a, _b, _c;
    if (!isFigmaFlipFrame(layer)) return false;
    const nodeType = (_b = (_a = layer.source) == null ? void 0 : _a.dataset) == null ? void 0 : _b.figmaNodeType;
    if (nodeType !== "FRAME") return false;
    const w = layer.box.width;
    const h = layer.box.height;
    if (w > 28 || h > 28) return false;
    return ((_c = layer.children) != null ? _c : []).some((c) => Boolean(c.vector));
  }
  ...
  async function buildLayer(layer, parent, grandparent, parentDocOrigin = { x: 0, y: 0 }) {
    ...
    if (isFigmaReferenceRasterLayer(layer)) {
      const node2 = createImageNode(layer);
      node2.name = layer.name || "reference-raster";
      if (!isMockFigmaRuntime()) {
        const pos = figmaReferenceRasterPosition(layer, parentDocOrigin);
        node2.x = pos.x;
        node2.y = pos.y;
    ...
    const isTextLeaf = layer.text && !isFigmaReferenceRasterLayer(layer) && (!layer.children || layer.children.length === 0);
    ...
    if (layer.paint) {
      ...
      if ("fills" in node && node.type !== "TEXT") {
        if (layer.image) {
          const bgFills = ((_ia = paint.fills) == null ? void 0 : _ia.length) ? buildFills(paint, layer.box.width, layer.box.height, layer) : void 0;
          ...
          const fills = ((_ja = paint.fills) == null ? void 0 : _ja.length) ? buildFills(paint, layer.box.width, layer.box.height, layer) : void 0;
  ...
  async function exportContentPng(canvas, _canvasBackground, exportScale) {
    var _a;
    const target = contentFrameFromCanvas(canvas);
    const scale = typeof exportScale === "number" && exportScale > 0 ? exportScale : isMockFigmaRuntime() ? 2 : 1;
    const settings = {
      format: "PNG",
