...
import { buildFigmaScreenPortfolioState, readScreenStepResult } from "./figma-screen-portfolio.mjs";
import {
  TEST_STEPS,
  TEST_STEP_ORDER,
  canRunStep,
...
function mapFigmaRow(figmaRow, repoRoot) {
  ...
  const leg = (stepId, legacyKey) => {
    const fromDisk = readScreenStepResult(repoRoot, screenId, stepId);
    const built = parityCellFromRec(repoRoot, fromDisk);
    if (built) return built;
    return c[stepId] ?? c[legacyKey] ?? { status: "not_tested", action: "Run parity test" };
  };
  return {
    entryPoint: "figma",
    storyId: screenId,
    cells: {
      ...
      vsFigmaLive: leg("vsFigmaLive", "contractFigma"),
      vsStorybook: leg("vsStorybook", "storybook"),
      vsReactHtml: leg("vsReactHtml", "fourWay"),
      logic: c.logic
    }
  };
}
