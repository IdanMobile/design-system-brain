 ...
 *
 * Fast fixer: cap parallel fix batches when shared importer is involved.
 */
...
/**
 * Effective parallel batch width for unified flow dispatch.
 * @param {string} repoRoot
 * @param {ReturnType<selectFlowWorkBatch>} batch
 */
export function effectiveFlowBatchLimit(repoRoot, batch) {
  if (process.env.FIXER_FAST_MODE === "0") {
    return Math.max(1, batch.length);
  }
 ...
 * @param {ReturnType<selectFlowWorkBatch>} batch
 */
export function filterParkedFromFlowBatch(repoRoot, batch) {
  return batch.filter((w) => !isStoryParked(repoRoot, w.storyId, w.stepId));
}
