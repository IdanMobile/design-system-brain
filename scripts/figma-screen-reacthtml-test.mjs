#!/usr/bin/env node
/**
 * Figma screen — Contract → ReactHtml (playground) render (pixel vs Guing reference PNG).
 *
 * Renders contract via render-html.ts in playground shell — no story map.
 *
 *   node scripts/figma-screen-reacthtml-test.mjs
 *   node scripts/figma-screen-reacthtml-test.mjs --artifact path/to.manifest.json
 */
...
const PLAYGROUND_SHELL = `${PLAYGROUND_URL}/?view=shell`;
...
async function testScreen({ manifestPath, pngPath }, tolerance) {
  ...
  if (manifestStep?.status !== "pass") {
    ...
    console.log(`  ✗ ${msg}`);
    writeScreenStepResult(WORKSPACE, name, "vsReactHtml", { status: "not_tested", error: msg });
  ...
  if (!existsSync(pngPath)) {
    ...
    console.log(`  ✗ ${msg}`);
    writeScreenStepResult(WORKSPACE, name, "vsReactHtml", { status: "error", error: msg });
    return { name, status: "error", error: msg };
  }
  ...
  try {
    ...
    const reactHtmlPath = join(itemDir, "reactHtml.png");
    const originalPath = join(itemDir, "original.png");
    const diffPath = join(itemDir, "diff-original-reactHtml.png");
    const blurVariantPath = join(itemDir, "reactHtml-blur.png");
    const noBlurVariantPath = join(itemDir, "reactHtml-noblur.png");

    const browser = await chromium.launch();
    const page = await browser.newPage();
    try {
      await screenshotContractHtml(page, doc, blurVariantPath, { shellUrl: PLAYGROUND_SHELL });
      await screenshotContractHtml(page, docNoBlur, noBlurVariantPath, { shellUrl: PLAYGROUND_SHELL });
    } finally {
      await browser.close();
    }
    ...
    writeScreenStepResult(WORKSPACE, name, "vsReactHtml", {
      status: gate.status,
      percent: gate.diffPct,
      maxRegionPercent: gate.regionGate.worst?.pct ?? null,
      pixelsDiffered: gate.diffPixels,
    ...
    writeScreenStepResult(WORKSPACE, name, "vsReactHtml", { status: "error", error: message });
...
async function main() {
  ...
  if (!targets.length) {
    console.log("[figma-screen-reacthtml] No manifests found");
    process.exit(0);
  }
  ...
  console.log(`\n[figma-screen-reacthtml] Done — ${results.length - failed}/${results.length} pass`);
  process.exit(failed ? 1 : 0);
}

main().catch((err) => {
  console.error("[figma-screen-reacthtml] Fatal:", err.message);
  process.exit(1);
});
