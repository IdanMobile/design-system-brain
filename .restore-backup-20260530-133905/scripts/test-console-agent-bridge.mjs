...
import { skillFollowLines as preambleSkillFollowLines, SKILLS, workflowPreamble, isFastFixerMode } from "./agent-workflow-preamble.mjs";
import {
...
export function initAgentBridge(repoRoot) {
  ...
  function buildCursorPrompt(worst, mode, extra = "") {
    const fixAllPrompt = extra.includes("Fix-all iteration");
    const skillCtx = {
      suiteId: worst.suiteId,
      storyId: worst.storyId,
      fastFixer: fixAllPrompt && isFastFixerMode(),
      fixAll: fixAllPrompt
    };
    if (isFigmaEntryFixSuite(worst.suiteId ?? worst.stepId)) {
      ...
      return [
        ...lines.slice(0, 1),
        "",
        ...skillFollowLines(entryMode === "live" ? "live" : "emulator", skillCtx),
        ...lines.slice(1)
      ].join("\n");
    }
    const lines = [
    ...
    if (!fixAllPrompt || !isFastFixerMode()) {
      lines.push(...sourceVsRendererTriageLines(mode, worst));
    }
    lines.push(...labMemoryHintLines(repoRoot, worst.storyId, mode, worst.suiteId));
    lines.push(...skillFollowLines(mode, skillCtx));
    if (!fixAllPrompt || !isFastFixerMode()) {
      lines.push(
        mode === "live"
  ...
  function buildFixAllStoryPrompt(story, mode, suiteId, attempt, maxTries, retry = null, supervisor = null) {
    ...
    const fast = isFastFixerMode();
    const modeLines =
      !fast && workerMode === "investigate_first"
        ? [
            "",
        ...
        : workerMode === "narrow_scope"
          ? [
              ...
              fast
                ? "Implement the cached root cause below in allowlisted adapter files only — one targeted edit."
