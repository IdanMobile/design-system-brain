...
import { isStoryParked } from "./test-console-worker-supervisor.mjs";
import {
  loadRunSettings,
...
export async function runPortfolioGolden(jobId, { killFlagPath, autoMode = false }) {
  ...
  while (!existsSync(killFlagPath)) {
    ...
    while (!getStepCompletion(suiteId).complete) {
      ...
      const allFailing = agent.findFailingStories(suiteId, SUITES, safeSegment);
      const failingStories = allFailing.filter((s) => !isStoryParked(ROOT, s.storyId, suiteId));
      const parkedCount = allFailing.length - failingStories.length;
      if (parkedCount > 0) {
        await appendLog(
          `[portfolio] Skipping ${parkedCount} parked stor${parkedCount === 1 ? "y" : "ies"} (see .test-console/parked-stories.json)\n`
        );
      }
