...
export async function runManagedAgent({
  ...
  const fastFixer = process.env.FIXER_FAST_MODE !== "0";
  /** Fast fixer / pixel: shorter budgets — adapter edits must land quickly. */
  const tightWatchdog =
    fastFixer || fixMode === "pixel"
      ? `AGENT_WATCHDOG_FIRST_EDIT_MS=${investigateFirst ? 6 * 60_000 : 5 * 60_000} AGENT_WATCHDOG_TOTAL_MS=${18 * 60_000} AGENT_WATCHDOG_MAX_BIG_READS=${fastFixer ? 8 : 20} `
      : "";
  const envPrefix = `${investigateFirst ? "AGENT_WATCHDOG_INVESTIGATE_MODE=1 " : ""}${tightWatchdog}`;
  const agentRunner = [
    `${envPrefix}node`,
