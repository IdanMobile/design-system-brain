...
/** Fast fixer: slim prompts + tight watchdogs. Set FIXER_FAST_MODE=0 for full role chain. */
if (process.env.FIXER_FAST_MODE === undefined) {
  process.env.FIXER_FAST_MODE = "1";
}
...
export async function runFixAllIterate(jobId, { killFlagPath, suiteId: suiteOverride, storyIds: storyIdsOverride } = {}) {
  ...
  await appendLog(
    fixAllSerialMode(storyIds)
      ? `[fix-all] Serial mode (default): up to ${MAX_TRIES_PER_STORY} fix→test cycles per story across ${storyIds.length} stor${storyIds.length === 1 ? "y" : "ies"} (${cfg.label})\n` +
          ...
          (process.env.FIXER_FAST_MODE !== "0"
            ? `[fix-all] Fast fixer ON — slim prompts, 5m first-edit / 18m total watchdog.\n`
            : "") +
  ...
  if (process.env.FIXER_FAST_MODE !== "0" && suiteUsesSharedImporter(suiteId)) {
    process.env.FIX_ALL_SERIAL = "1";
    await appendLog(
      `[fix-all] Shared adapter suite (${suiteId}) — forcing FIX_ALL_SERIAL=1 (no parallel code-v2 edits).\n`
    );
  }

  const parkedBefore = serialStoryIds.filter((id) => isStoryParked(ROOT, id, suiteId));
  serialStoryIds = filterOutParkedStories(ROOT, serialStoryIds, suiteId);
  if (parkedBefore.length) {
    await appendLog(
      `[fix-all] Skipping ${parkedBefore.length} parked stor${parkedBefore.length === 1 ? "y" : "ies"} (exhausted earlier — fix by hand or delete .test-console/parked-stories.json): ${parkedBefore.join(", ")}\n`
    );
  }
  if (!serialStoryIds.length) {
    await appendLog("[fix-all] All requested stories are parked — nothing to fix.\n");
    return { exitCode: 0, passed: false, summary: "all parked" };
  }
  ...
  for (let i = 0; i < serialStoryIds.length; i++) {
    ...
    if (
      current?.error &&
      String(current.error).includes("Export timed out") &&
      process.env.FIXER_FAST_MODE !== "0"
    ) {
      parkExhaustedStory(ROOT, storyId, suiteId, {
        reason: "export_timeout",
        metrics: metricsFromStory(storyMeta)
      });
      await appendLog(
        `${prefix} — parked (Figma export timeout). Fix relay/plugin/infra, re-test manually, then clear park entry.\n`
      );
      storiesExhausted += 1;
      continue;
    }
    ...
    for (let attempt = 1; attempt <= MAX_TRIES_PER_STORY; attempt++) {
      ...
      const supervisorForPrompt = lastSupervisor
        ...
        : null;
      const workerModeRaw = supervisorForPrompt?.nextWorkerMode ?? "continue";
      const invGate = investigatorGateAllowsFixer(ROOT, storyId, suiteId);
      let workerMode = workerModeRaw;
      if (process.env.FIXER_FAST_MODE !== "0") {
        if (invGate.allowed && workerMode === "investigate_first") {
          workerMode = "narrow_scope";
        }
        if (invGate.allowed) {
          await appendLog(
            `${prefix} attempt ${attempt} — fixer (cached root cause in lab-memory; skipping investigator)\n`
          );
        } else if (attempt === 1) {
          await appendLog(
            `${prefix} attempt ${attempt} — no cached root cause; edit after opening compare PNG only (no skill reads)\n`
          );
        }
      } else if (workerMode === "investigate_first") {
        await appendLog(
          `${prefix} attempt ${attempt} — INVESTIGATOR phase (fixer blocked: ${invGate.reason})…\n`
        );
      }
      ...
      const prompt = agent.buildFixAllStoryPrompt(
        ...
        lastAttemptOutcome,
        supervisorForPrompt
          ? { ...supervisorForPrompt, nextWorkerMode: workerMode }
          : null
      );
      const storyAgent = normalizeAgentResult(
        await runManagedAgent({
          ...
          killFlagPath,
          investigateFirst:
            process.env.FIXER_FAST_MODE === "0" && workerMode === "investigate_first",
          fixMode: cfg.mode === "figmaEntry" ? "live" : cfg.mode
        })
      );
    ...
    if (passed) {
      ...
      if (regressionOk) {
        clearParkedStory(ROOT, storyId, suiteId);
        storiesPassed += 1;
      } else {
        passed = false;
      }
      ...
      parkExhaustedStory(ROOT, storyId, suiteId, {
        reason: "exhausted",
        metrics: current ?? metricsFromStory(storyMeta)
      });
      await appendLog(
        `${prefix} — gave up after ${MAX_TRIES_PER_STORY} attempts (parked — clear .test-console/parked-stories.json to retry)\n`
      );
    }
  }
