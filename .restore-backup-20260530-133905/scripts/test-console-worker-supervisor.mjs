...
export function evaluateAttempt(input) {
  ...
  if (attempt >= 2 && globalSame && hotspotSame) {
    ...
    const cached = repoRoot ? loadLabMemoryFixHint(repoRoot, storyId, suiteId) : null;
    if (cached && nextWorkerMode === "continue") {
      nextWorkerMode = "narrow_scope";
      interventionLines.push(
        "Supervisor: metrics unchanged but lab-memory has root cause — implement the cached fix in allowlisted adapter files; do not re-investigate."
      );
    } else if (nextWorkerMode === "continue") {
      nextWorkerMode = "investigate_first";
      interventionLines.push(
        "Supervisor: metrics unchanged across attempts — stop guessing; use compare PNG + artifact before editing."
      );
    }
  }
    ...
    if (prevAfter && globalSame && Math.abs((prevAfter.percent ?? 0) - afterTest.percent) <= 0.001) {
      if (!verdicts.includes("STUCK_LOOP")) {
        ...
        if (cached && nextWorkerMode === "continue") {
          nextWorkerMode = "narrow_scope";
          interventionLines.push(
            "Supervisor: two flat attempts with cached root cause — edit allowlisted adapter only."
          );
        } else if (nextWorkerMode === "continue") {
          nextWorkerMode = "investigate_first";
          interventionLines.push(
...
/**
 * @param {string} repoRoot
 * @returns {{ updatedAt: string, items: object[] }}
 */
...
/**
 * Investigator gate — fixer may run when lab-memory has a filled root cause block.
 * @param {string} repoRoot
 * @param {string} storyId
 * @param {string} suiteId
 */
export function investigatorGateAllowsFixer(repoRoot, storyId, suiteId) {
 ...
 * @param {string} jobId
 * @param {string} storyId
 * @param {"investigator"|"fixer"} phase
 * @param {number} attempt
 * @param {object} payload
 */
export function writeStructuredJobResult(repoRoot, jobId, storyId, phase, attempt, payload) {
  ...
  mkdirSync(dir, { recursive: true });
  const safeStory = storyId.replace(/[<>:"/\\|?*]/g, "-");
  const path = join(dir, `${safeStory}-${phase}-try-${attempt}.json`);
  writeFileSync(
    path,
    JSON.stringify(
      {
