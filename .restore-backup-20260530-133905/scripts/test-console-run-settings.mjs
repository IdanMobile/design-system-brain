...
export function normalizeRunSettings(raw = {}) {
  ...
  const scope = String(raw.scope ?? "failures_only");
  const validScope = ["full", "failures_only", "fresh_only", "single_step"].includes(scope)
    ? scope
    : "failures_only";
  const sortBy = String(raw.sortBy ?? "step_first");
  return {
    skipPass: raw.skipPass !== false,
    ...
    scope: validScope,
    singleStepId: raw.singleStepId ? String(raw.singleStepId) : null,
    sortBy: sortBy === "worst_first" ? "worst_first" : "step_first",
