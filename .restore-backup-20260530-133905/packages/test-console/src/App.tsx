...
function pctColumnLabel(stepId: string): string {
  if (stepId === "structural") return "Extract";
...
export function App() {
  ...
  const launchOrchestrator = async (
    ...
    if (isPortfolioOrchestratorRunning) {
    ...
    try {
      ...
      const entry: RunningJobEntry = {
        jobId: data.jobId,
        actionId: PORTFOLIO_ORCHESTRATOR_ACTION,
        label: data.label ?? "Orchestrator · unified portfolio",
        startedAt: new Date().toISOString(),
        status: "running"
      };
      setRunningJobs((prev) => ({ ...prev, [data.jobId!]: entry }));
      if (data.terminalDispatched === false) {
        setApiError(
  ...
  const handleRowFix = (fixSuite: string, storyId: string, _entryPoint?: string) => {
  ...
  return (
    <div className="app">
      ...
      <header>
        <div className="header-row">
          ...
          <div className="header-orchestrator">
            <button
              ...
              disabled={isPortfolioOrchestratorRunning && !portfolioOrchestratorFinished}
              title={
                isPortfolioOrchestratorRunning
                  ? portfolioOrchestratorJob()?.progress?.logTail ??
                    "Orchestrator running in Terminal"
                  : "Configure scope, agent, and safety limits — then Launch"
              }
        ...
        {workerSupervisor?.finished && workerSupervisor.humanMessage ? (
          ...
          <section className="card" style={{ marginTop: 16 }}>
            ...
            {activePortfolio && activePortfolio.rows.length > 0 ? (
              ...
              <p style={{ color: "var(--muted)" }}>
                No portfolio yet. Run <code>pnpm test:portfolio:refresh</code> or a parity test
                suite.
              </p>
            )}
          </section>
