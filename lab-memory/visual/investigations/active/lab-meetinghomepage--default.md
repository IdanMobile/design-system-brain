...
Radial gradient **ellipse size** (`120% 60%`, `80% 40%`) was dropped from schema/render — fixed in `extract.ts` + `render-html.ts` (2026-05-25). Remaining ~0.64% global diff is spread across header gradient fringe, live-card column, bottom nav, and earlier-meetings column (see region PNGs). Likely sub-pixel text/layout on cards and nav icons, not a single missing layer.

### Recommended fix area

Compare `pixel-diffs/lab-meetinghomepage-default/regions/region-02-compare.png` and `region-03-compare.png`; tune `render-html.ts` flex/text or card borders if hotspots are structural.

### Cached

false — automated test record at 2026-05-25T08:39:23.498Z

<!-- vault-fingerprint: pixel|fail|0.647|na|5|fix-all pre-agent -->

## Investigation — 2026-05-25 (manual)

**Harness:** `investigate_first` + 8m no-edit watchdog conflict fixed (`AGENT_WATCHDOG_INVESTIGATE_MODE`, 14m budget). Fix-all queue now worst-first.

**Pixel after shared fixes:** still **fail 0.644%** (needs ≤0.1%). `lab-spacemissioncontrol--default` now **pass**.
...
<!-- vault-fingerprint: pixel|fail|0.644|na|0|fix all requested -->

## Investigation — lab-meetinghomepage--default / pixel

**Job ID:** n/a  
**Date:** 2026-05-25T14:41:34.025Z  
**Source:** fix all requested (automated)
