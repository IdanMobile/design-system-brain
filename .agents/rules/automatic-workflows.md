...
| --- | --- |
| **Fix** — `run until pass`, `make fixes after live test`, test-console inbox, `pnpm test:console:cursor agent` | 1. `project-orchestrator` pre-flight (phase, infra, portfolio) 2. **Lab memory:** read `lab-memory/visual/patterns/` + `visual/investigations/active/<storyId>.md` 3. `systematic-debugging` + `investigate-figma-mismatch` **before** edits 4. **Append** investigation (`lab-memory/templates/investigation.md`) 5. `figma-renderer-until-pass` implement 6. Tier **A** (re-run steps 1..N for story) 7. Tier **C** if `code-v2.ts`, `scene-to-html.ts`, `extract.ts`, or `contract` changed 8. `verification-before-completion` with command output |
