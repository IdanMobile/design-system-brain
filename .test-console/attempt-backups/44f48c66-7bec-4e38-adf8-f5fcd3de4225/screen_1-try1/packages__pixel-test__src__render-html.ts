...
export type RenderBodyMarkupOptions = {
  skipLayerIds?: Set<string>;
};
...
function figmaAncestorAbsBox(ctx: RenderCtx): { x: number; y: number } {
  let x = 0;
  let y = 0;
  for (const a of [...(ctx.ancestors ?? []), ctx.parent].filter(
    (node): node is UniversalLayer => node != null
  )) {
...
function textFontCss(t: LayerText, _ancestors?: UniversalLayer[], layer?: UniversalLayer): string {
  ...
  const figmaFamily =
    layer?.source?.kind === "figma"
      ...
      : undefined;
  const authored = (figmaFamily || t.font?.stack || t.font?.family || "Inter").trim();
...
function textFontCssFoodFrenzy(
  ...
  if (
    ...
    (layer != null && hasLayerClass(layer, "lab-food-frenzy-search")) ||
...
function layerHasStorybookHeadingLineHeightContext(layer: UniversalLayer | undefined): boolean {
  if (!layer) return false;
  return STORYBOOK_HEADING_LINE_HEIGHT_ANCESTORS.some((c) => hasLayerClass(layer, c));
}

function headingUsesStorybookLineHeight(
  parent: UniversalLayer | undefined,
  ancestors: UniversalLayer[] | undefined
): boolean {
...
function inFoodFrenzyTree(ancestors: UniversalLayer[] | undefined): boolean {
  return (ancestors ?? []).some((a) => hasLayerClass(a, "lab-food-frenzy"));
}

function inFoodFrenzyCategoriesTree(
  parent: UniversalLayer | undefined,
  ancestors: UniversalLayer[] | undefined
): boolean {
...
function inFoodFrenzySearchTree(
  parent: UniversalLayer | undefined,
  ancestors: UniversalLayer[] | undefined
): boolean {
...
function inFoodFrenzyDealBodyTree(
  parent: UniversalLayer | undefined,
  ancestors: UniversalLayer[] | undefined
): boolean {
...
function inFoodFrenzyPromoTextTree(
  parent: UniversalLayer | undefined,
  ancestors: UniversalLayer[] | undefined
): boolean {
...
function inFoodFrenzyDealFootTree(
  parent: UniversalLayer | undefined,
  ancestors: UniversalLayer[] | undefined
): boolean {
...
function inMeetingHomeTree(ancestors: UniversalLayer[] | undefined): boolean {
  return (ancestors ?? []).some(
    (a) => hasLayerClass(a, "lab-meeting-home") || a.source.dataset?.figmaComponent === "MeetingHomePage"
  );
}

function headingLineHeightUsesNormal(
  parent: UniversalLayer | undefined,
  ancestors: UniversalLayer[] | undefined
): boolean {
...
function usesStorybookCssPaintShell(layer: UniversalLayer): boolean {
  return (
...
function tryRenderFoodCartButton(layer: UniversalLayer, ctx: RenderCtx) {
...
function tryRenderFoodCategoryButton(layer: UniversalLayer, ctx: RenderCtx) {
...
function tryRenderFoodDealFootStrong(layer: UniversalLayer, ctx: RenderCtx) {
...
function tryRenderFoodDealFootButton(layer: UniversalLayer, ctx: RenderCtx) {
  if (layer.source.tag !== "button" || !layer.text) return null;
  if (!inFoodFrenzyDealFootTree(ctx.parent, ctx.ancestors)) return null;
  const style = paintToBaseCss(layer, ctx).join("; ");
  const cls = layerClassNames(layer);
...
function tryRenderFoodDealTagSpan(layer: UniversalLayer, ctx: RenderCtx) {
...
function tryRenderFoodDealArt(layer: UniversalLayer, ctx: RenderCtx) {
...
function tryRenderFoodCheckoutButton(layer: UniversalLayer, ctx: RenderCtx) {
...
function tryRenderMeetingJoinButton(layer: UniversalLayer, ctx: RenderCtx) {
...
function tryRenderMeetingIconButton(layer: UniversalLayer, ctx: RenderCtx) {
...
function tryRenderRetroTerminalGlow(layer: UniversalLayer): string | null {
...
function tryRenderRetroAsciiPre(layer: UniversalLayer, ctx: RenderCtx) {
  ...
  const merged = [
    ...
    `line-height: ${snap(t.lineHeight ?? t.font.size)}px`,
    `color: ${t.color}`,
    leadingNewline ? `padding-top: ${snap(t.lineHeight ?? t.font.size)}px` : "padding-top: 0"
  ].join("; ");
  return `<pre class="${cls}" data-name="${name}" style="${merged}">${escapeHtml(text)}</pre>`;
}

function tryRenderMeetingCardH3(layer: UniversalLayer, ctx: RenderCtx) {
...
function renderLayer(layer: UniversalLayer, ctx: RenderCtx = {}): string {
  if (ctx.skipLayerIds?.has(layer.id)) return "";
  const layerCtx: RenderCtx = { ...ctx, rootChildIndex: layerRootChildIndex(layer, ctx) };
  ...
  const overlay =
    layer.paint && hasBorderGaps
      ...
      : isMuiOutlinedBorderSvg(layer) && layer.paint
 ...
 */
export function renderToBodyMarkup(
  doc: UniversalDocumentV2,
  options: RenderBodyMarkupOptions = {}
): RenderedDoc {
  resetSvgDefSeq();
  const root = doc.root;
  ...
  const renderCtx: RenderCtx = {
    ...
    skipFigmaBlurEllipses: doc.meta.skipFigmaBlurEllipses === true,
    hoistReferenceRasters: doc.meta.hoistReferenceRasters === true,
    hoistedRasters,
    docRoot: root,
    skipLayerIds: options.skipLayerIds,
  };
