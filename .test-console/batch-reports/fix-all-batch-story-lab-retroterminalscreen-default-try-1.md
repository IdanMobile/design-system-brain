...
### `snap` — `packages/pixel-test/src/render-html.ts` (lines 21–61 of 2917)
...
/** Match Storybook CSS keywords in gradient stops (e.g. `transparent` vs `rgba(0,0,0,0)`). */
function gradientStopColor(color: string): string {
  const c = color.trim().toLowerCase();
  if (c === "rgba(0, 0, 0, 0)" || c === "rgba(0,0,0,0)") return "transparent";
```
