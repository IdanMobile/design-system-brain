...
### `buildLayer` — `packages/figma-importer-plugin/src/code-v2.ts` (lines 2847–2887 of 3674)
...
async function buildLayer(
  ...
  labRoot?: UniversalLayer
): Promise<SceneNode | null> {
  if (!isLayerVisible(layer)) return null;

  const effectiveLabRoot = resolveLabRoot(layer, labRoot);
  ...
  if (figmaBareText) {
    const t = await createTextNode(layer, parent, effectiveLabRoot);
...
### `applyTransform` — `packages/figma-importer-plugin/src/code-v2.ts` (lines 1057–1097 of 3674)
...
### `applyBorders` — `packages/figma-importer-plugin/src/code-v2.ts` (lines 1173–1213 of 3674)
...
### `applyCornerRadii` — `packages/figma-importer-plugin/src/code-v2.ts` (lines 1161–1201 of 3674)
...
### `clampNodeWidthToParent` — `packages/figma-importer-plugin/src/code-v2.ts` (lines 58–98 of 3674)
...
### `snap` — `packages/figma-importer-plugin/src/code-v2.ts` (lines 43–83 of 3674)
...
### `snapBoxSize` — `packages/figma-importer-plugin/src/code-v2.ts` (lines 49–89 of 3674)
...
### `snap` — `packages/pixel-test/src/render-html.ts` (lines 21–61 of 3136)
