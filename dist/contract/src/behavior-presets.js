...
export function applyPresetTemplate(presetId, displayName) {
    const preset = BEHAVIOR_PRESETS.find((p) => p.id === presetId);
    if (!preset || preset.id === "custom" || !preset.descriptionTemplate)
        return "";
...
export function inferPresetFromDescription(description) {
    const d = description.toLowerCase().trim();
    if (!d)
        return "custom";
    if (/\b(hover|mouse over)\b/.test(d))
        return "hover";
    if (/\b(search)\b/.test(d))
        return "search";
    if (/\b(type|types|enter|edit)\b/.test(d) && /\b(field|input|text)\b/.test(d))
        return "edit-text";
    if (/\b(select|pick|choose)\b/.test(d))
        return "select";
    if (/\b(submit)\b/.test(d))
        return "submit";
    if (/\b(loading|spinner)\b/.test(d))
        return "loading";
    if (/\b(navigate|go to|route)\b/.test(d))
        return "navigate";
    if (/\b(toggle|switch)\b/.test(d))
        return "toggle";
    if (/\b(data|display|show)\b/.test(d) && !/\b(click)\b/.test(d))
        return "data";
    if (/\b(click|tap|press)\b/.test(d))
        return "click";
    return "custom";
}
