 ...
 */
export async function extractStoryV2(storyId, out, baseUrl = "http://127.0.0.1:6107", argsUsed) {
    const argsQuery = argsUsed && Object.keys(argsUsed).length ? `&args=${serializeArgs(argsUsed)}` : "";
    ...
    const imageUrlPromises = new Map();
    ...
    const tree = await page.evaluate(({ storyId: sid, argsUsed: usedArgs }) => {
        const px = (v) => {
            if (!v)
                return 0;
        ...
        function parseTransformMatrix(raw) {
            if (!raw || raw === "none")
                return undefined;
        ...
        function parseFilters(raw) {
            ...
            while ((match = re.exec(raw)) !== null) {
                ...
                if (fn === "blur") {
                    ...
                    if (s)
                        out.push({ kind: "drop-shadow", shadow: s });
                }
                else if (["brightness", "contrast", "grayscale", "invert", "saturate", "sepia", "opacity"].includes(fn)) {
        ...
        function parseRadialGradient(raw) {
        ...
        function parseConicGradient(raw) {
            ...
            if (head.startsWith("from ") || head.startsWith("at ")) {
                ...
                if (fromMatch)
                    fromDeg = Number.parseFloat(fromMatch[1]);
        ...
        function parseBackgroundLayers(style) {
            const fills = [];
            ...
            for (let i = 0; i < layers.length; i += 1) {
                ...
                const sizeParsed = sizeRaw === "cover" || sizeRaw === "contain" || sizeRaw === "auto"
                    ? sizeRaw
                    : (() => {
        ...
        function borderSide(widthRaw, colorRaw, styleRaw) {
            const w = px(widthRaw);
            if (w <= 0)
                return undefined;
            if (!styleRaw || styleRaw === "none" || styleRaw === "hidden")
                return undefined;
            return { width: w, color: colorRaw, style: styleRaw };
        }
        function buildBorders(style, legendGap) {
            const top = borderSide(style.borderTopWidth, style.borderTopColor, style.borderTopStyle);
            const right = borderSide(style.borderRightWidth, style.borderRightColor, style.borderRightStyle);
            const bottom = borderSide(style.borderBottomWidth, style.borderBottomColor, style.borderBottomStyle);
            const left = borderSide(style.borderLeftWidth, style.borderLeftColor, style.borderLeftStyle);
            if (!top && !right && !bottom && !left && !legendGap)
                return undefined;
            const result = {};
            if (top)
                result.top = top;
        ...
        function buildCorners(style) {
            ...
            if (!tlx && !tly && !trx && !tryp && !brx && !bry && !blx && !bly)
                return undefined;
            return {
                topLeft: { x: tlx, y: tly },
        ...
        function isHidden(el) {
            ...
            if (clip.includes("rect(0px, 0px, 0px, 0px)") || clip.includes("inset(50%)"))
                return true;
            const classText = (html.getAttribute("class") || "").toString();
            if (/(^|\s)(visuallyhidden|sr-only)(\s|$)/i.test(classText))
                return true;
            return false;
        }
        function visibleTextValue(html) {
        ...
        function isTextLeaf(el) {
            const html = el;
            const tag = html.tagName.toLowerCase();
            if (["script", "style", "br"].includes(tag))
                return false;
        ...
        function svgPaint(el) {
            ...
            if (sw && sw !== "0px")
                paint.strokeWidth = px(sw);
            const opacity = cs.opacity;
            if (opacity && Number(opacity) < 1)
                paint.opacity = Number(opacity);
            const fo = cs.fillOpacity;
            if (fo && Number(fo) < 1)
                paint.fillOpacity = Number(fo);
            const so = cs.strokeOpacity;
            if (so && Number(so) < 1)
                paint.strokeOpacity = Number(so);
            const lc = cs.strokeLinecap;
            if (lc && lc !== "butt")
                paint.lineCap = lc;
            const lj = cs.strokeLinejoin;
            if (lj && lj !== "miter")
                paint.lineJoin = lj;
            const fr = cs.fillRule;
            if (fr && fr !== "nonzero")
                paint.fillRule = fr;
            ...
            const dashOffsetRaw = cs.strokeDashoffset;
            if (dashOffsetRaw && dashOffsetRaw !== "0px" && dashOffsetRaw !== "0") {
                paint.dashOffset = px(dashOffsetRaw);
            }
            return paint;
        }
        function svgShape(el) {
            const tag = el.tagName.toLowerCase();
            const primitive = tag === "path" || tag === "rect" || tag === "circle" || tag === "ellipse" ||
            ...
            if (primitive === "text") {
                shape.text = { value: el.textContent || "" };
            }
            return shape;
        }
        function buildVectorSpec(svg) {
            ...
            if (par)
                out.preserveAspectRatio = par;
            return out;
        }
        // ────────────────────── pseudo-element capture ──────────────────────
        function pseudoLayer(host, pseudo, hostRect, diagnostics) {
            let cs;
            try {
            ...
            if (!content || content === "none" || content === "normal") {
                ...
                if (cs.display === "none")
                    return null;
                if ((px(cs.width) <= 0 || px(cs.height) <= 0) &&
            ...
            if (transform && !isIdentityMatrix(transform))
                layer.transform = { matrix: transform };
        ...
        function buildPaintFromComputed(style, legendGap) {
            ...
            const backdropFilters = parseFilters(style.backdropFilter || "");
        ...
        // ─────────────────────── computed style → text ───────────────────────
        function pickFontFamily(familyRaw) {
        ...
        function resolveLineHeight(style, el) {
            ...
            try {
                ...
                if (r.height > 0 && r.height <= fs * 1.5)
                    return px(String(r.height));
            }
            catch {
                /* ignore */
            }
        ...
        function isSystemUiFont(familyRaw) {
            const first = familyRaw.split(",")[0]?.trim().replace(/['"]/g, "") || "";
            return /^arial\b|^system-ui\b|^-apple-system\b|^\.?apple-system\b|^ui-sans-serif\b/i.test(first);
        }
        ...
        function buildTextSpec(style, value, diagnostics, _layerId, el) {
            const measureEl = el ? textMeasureEl(el) : el;
            const measureStyle = measureEl && measureEl !== el ? getComputedStyle(measureEl) : style;
            ...
            const lines = [];
            if (decoLine.includes("underline"))
                lines.push("underline");
            if (decoLine.includes("line-through"))
                lines.push("line-through");
            if (decoLine.includes("overline"))
                lines.push("overline");
            const decoration = lines.length
                ? {
                    ...
                    thicknessPx: px(style.textDecorationThickness)
                }
                : undefined;
        ...
        function buildLayoutSpec(style) {
            ...
            const layout = { display, position, overflow };
            layout.padding = {
            ...
            if (style.top && style.top !== "auto")
                inset.top = px(style.top);
            if (style.right && style.right !== "auto")
                inset.right = px(style.right);
            if (style.bottom && style.bottom !== "auto")
                inset.bottom = px(style.bottom);
            if (style.left && style.left !== "auto")
                inset.left = px(style.left);
            if (Object.keys(inset).length)
                layout.inset = inset;
            const z = style.zIndex;
            if (z && z !== "auto")
                layout.zIndex = Number(z) || 0;
            ...
            if (Number.isFinite(fs) && fs !== 1)
                layout.flexShrink = fs;
            if (style.flexBasis && style.flexBasis !== "auto")
                layout.flexBasis = style.flexBasis;
            if (style.order && style.order !== "0")
                layout.order = Number(style.order) || 0;
            if (style.alignSelf && style.alignSelf !== "auto")
                layout.alignSelf = style.alignSelf;
            if (style.justifySelf && style.justifySelf !== "auto")
                layout.justifySelf = style.justifySelf;
            if (style.gridColumn && style.gridColumn !== "auto")
                layout.gridColumn = style.gridColumn;
            if (style.gridRow && style.gridRow !== "auto")
                layout.gridRow = style.gridRow;
            return layout;
        }
        // ───────────────────── legend → border gap detection ─────────────────────
        function detectLegendGap(el, rect) {
        ...
        function captureSource(el) {
            ...
            if (html.id)
                source.id = html.id;
            if (classListArr.length)
                source.classList = classListArr;
        ...
        const diagnostics = [];
        const warnings = [];
        function walk(el, parentRect, parentDisplay) {
            if (isHidden(el))
                return null;
            const html = el;
            const tag = el.tagName.toLowerCase();
            if (tag === "br" || tag === "script" || tag === "style" || tag === "noscript")
                return null;
            if (tag === "input") {
                const t = html.type?.toLowerCase() || "";
                if (t === "hidden")
                    return null;
            }
            ...
            if (parentDisplay === "grid") {
                ...
                if (box.width > maxW + 0.5)
                    box.width = Math.max(1, maxW);
            }
            const computedBox = {
                x: snap(rect.left),
                y: snap(rect.top),
            ...
            if (transformMatrix && !isIdentityMatrix(transformMatrix)) {
                ...
                if (origin)
                    layer.transform.origin = parseOrigin(origin);
            }
            // SVG capture (recursive, per-primitive paint)
            if (tag === "svg") {
                layer.vector = buildVectorSpec(el);
                return layer;
            }
            // Image capture
            if (tag === "img") {
            ...
            if (!["input", "textarea", "select"].includes(tag) &&
                ...
                if (directText) {
                    ...
                    const textNodes = Array.from(html.childNodes).filter((n) => n.nodeType === Node.TEXT_NODE && (n.textContent || "").trim().length > 0);
                    const ranges = textNodes.length > 0
                        ? textNodes.map((n) => {
        ...
        function getCanvasBackground(start) {
        ...
        const rootEl = document.querySelector("[data-figma-component]");
        if (!rootEl)
            throw new Error("No [data-figma-component] root found");
        const rootRect = rootEl.getBoundingClientRect();
        const canvasBg = getCanvasBackground(rootEl);
        const root = walk(rootEl, rootRect);
        if (!root)
            throw new Error("Root element walk produced no layer");
        // Collect all image URLs to fetch outside (data URL pass)
        const imageUrls = new Set();
        function collectImages(layer) {
            if (layer.image?.src)
                imageUrls.add(layer.image.src);
    ...
    const dataUrlByImage = new Map();
    await Promise.all(tree.imageUrls.map(async (url) => {
        const data = await fetchAsDataUrl(url);
        if (data)
            dataUrlByImage.set(url, data);
    }));
    function attachImageData(layer) {
...
if (isDirectRun) {
    ...
    const parsedArgs = argsRaw ? JSON.parse(argsRaw) : undefined;
