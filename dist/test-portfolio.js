...
export const PORTFOLIO_STORY_IDS = DEV_STORIES.map((s) => s.id);
/** Sequential pipeline order — step N requires all prior steps to pass. */
export const TEST_STEP_ORDER = [
    "pixel",
    "figma",
    "figmaLive",
    "delivery",
...
export function recommendAction(stepId, status, detail) {
    ...
    if (status === "fail") {
        if (stepId === "figmaLive")
            return "Fix renderer — reload plugin, re-run live";
        if (stepId === "figma")
        ...
        if (stepId === "logic")
            return "Fix audit harness / Delivery showcase load";
        return "Fix pipeline";
    }
    if (status === "warn") {
        ...
        if (stepId === "figma")
            return `Tighten mock renderer${pct}`;
        if (stepId === "figmaLive")
            return `Tighten live export${pct}`;
